...
export const api = {
  ...
  async getGroupOverview(
    ...
    const res = await fetch(
      `${API_BASE}/group${queryString ? `?${queryString}` : ""}`,
    );
  ...
  async getDealerPrincipalCluster(
    clusterId: string,
    params: { page?: number; limit?: number } = {},
  ): Promise<DealerPrincipalData> {
    const query = new URLSearchParams();
    if (params.page) query.set("page", params.page.toString());
    if (params.limit) query.set("limit", params.limit.toString());
    const queryString = query.toString();
    const res = await fetch(
      `${API_BASE}/dp/${clusterId}${queryString ? `?${queryString}` : ""}`,
    );
  ...
  async getGeneralManagerRooftop(
    ...
    const queryString = query.toString();
    const res = await fetch(
      `${API_BASE}/gm/${rooftopId}${queryString ? `?${queryString}` : ""}`,
    );
