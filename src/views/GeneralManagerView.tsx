...
export const GeneralManagerView: React.FC<GeneralManagerViewProps> = ({
  ...
  }, [rooftopId, categoryFilter, inventoryPage, inventoryLimit]);
  ...
  const inventoryPagination = data.inventoryPagination ?? {
    total: data.inventoryList.length,
    page: inventoryPage,
    limit: inventoryLimit,
    totalPages: Math.max(
      1,
      Math.ceil(data.inventoryList.length / inventoryLimit),
    ),
  };
  const displayedInventory = data.inventoryPagination
    ? data.inventoryList
      ...
      <div className="rounded-xl border border-surface-border bg-surface-card overflow-hidden">
        <div className="p-4 border-b border-surface-border flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Showing all {inventoryPagination.total} units currently on lot
            </p>
          </div>
